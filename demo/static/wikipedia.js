var WIKIPEDIA = function() {
  ...
  my._getDbpediaUrl = function(url) {
    if (url.indexOf('wikipedia')!=-1) {
      ...
      return url;
    } else if (url.indexOf('dbpedia.org')!=-1 || url.indexOf('wikidata')!=-1) {
      return url;
    } else {
  ...
  my.getRawJson = function(url, callback, error) {
    var sparqlQuery = `DESCRIBE <${url}>`;

    let request_url = new URL(my.endpoint);
    let params = {
      query: sparqlQuery,
      format: 'application/rdf+json'
    };

    request_url.search = new URLSearchParams(params).toString()
